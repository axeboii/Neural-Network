...
class Helpfuncs:
    # Tests the network with given number of images, returns loss and accuracy
    def test(NN, numOfPredicts, test_X, test_y):
        ...
        for i in range(0,numOfPredicts):
            ...
            xk = test_X[randomInt]
            yk = test_y[randomInt]
            predictionVec = NN.predict(xk)
            lossFuncSum += Helpfuncs.lossFunc(predictionVec, yk)
            prediction = max(predictionVec)
            index = predictionVec.index(prediction)
            if (index == int(yk)):
                correct += 1
        acc =  correct/numOfPredicts
        loss = lossFuncSum/numOfPredicts
        return acc, loss
