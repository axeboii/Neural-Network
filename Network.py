...
# Global variables
SIZES = [28*28, 400, 400, 10] # Number of nodes in: [input layer, hidden layer, ... , output layer]
EPOCHS = 103
BATCH_SIZE = 16
...
class Network:
    ...
    def updateLearningRate(self, epoch):
        if self.sc == "exponentialDecay":
            self.lr = hf.exponentialDecay(self.lr, self.dr)
        elif self.sc == "polynomialDecay":
            self.lr = hf.polynomialDecay(self.lr, END_LEARNING_RATE, epoch, EPOCHS)
        elif self.sc == "inverseTimeDecay":
            self.lr = hf.inverseTimeDecay(self.lr, self.dr, epoch)
        elif self.sc == "piecewiseConstantDecay":
            self.lr = hf.piecewiseConstantDecay(self.lr, epoch)
        else: 
            pass
    ...
    def writeLossFunc(self):
        ...
        while os.path.exists(path+fileEnd):
            path = path + ".1"        
        open(path + fileEnd, 'w').close() # Clears the file
        with open(path+fileEnd, 'a') as myfile:
            myfile.writelines(str(i)+"\n" for i in self.lossFuncValues)
